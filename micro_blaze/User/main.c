...
static void uart_puts(const char *s) {
    fe_port_uart_write(0, (const uint8_t *)s, strlen(s));
}

static void print_help(void) {
    char names[512];
    uart_puts("Usage: <data|ability>_<Name> <act> [args]\n");
    ...
    uart_puts(names);
    uart_puts("\nexamples:\n");
    uart_puts("  ability_BaseAbility list_ability_names\n");
    uart_puts("  ability_TimeAbility sync_ntp\n");
    uart_puts("  ability_ModbusAbility write_holding 0,42\n");
    uart_puts("  data_ConfigData set wifi.ssid=MyNet\n");
}
...
int main(void) {
    ...
    const char *banner = "\nFasterEdge-FPGA (Zynq-7020 ALINX / MicroBlaze)\ninput: <data|ability>_<Name> <act> [args]  |  'help'\n";
